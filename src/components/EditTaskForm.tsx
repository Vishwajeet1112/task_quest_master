...
const EditTaskForm = ({ task, onUpdateTask, onCancel }: EditTaskFormProps) => {
  const [title, setTitle] = useState(task.title);
  const [description, setDescription] = useState(task.description || "");
  const [difficulty, setDifficulty] = useState<'easy' | 'medium' | 'hard'>(task.difficulty);
  const [category, setCategory] = useState<'work' | 'personal' | 'health' | 'learning'>(task.category);
  const [photo, setPhoto] = useState<string>(task.photo || "");
  ...
  return (
    <Card className="glass border-2">
      <CardHeader>
        <CardTitle className="gradient-text">Edit Quest</CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              >
                ...
                {isRecording ? 'Stop Recording' : audio ? 'Re-record Audio' : 'Record Audio'}
              </Button>
            </div>
